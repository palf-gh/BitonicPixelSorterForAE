...
$benchCandidates = @(
	(Join-Path $repoRoot "build\Win-perf\tools\$Config\bps_dx_axis_bench.exe"),
	(Join-Path $repoRoot "build\Win\tools\$Config\bps_dx_axis_bench.exe"),
	(Join-Path $repoRoot "build\Win-perf\$Config\bps_dx_axis_bench.exe"),
	(Join-Path $repoRoot "build\Win\$Config\bps_dx_axis_bench.exe")
)

$benchExe = $benchCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $benchExe) {
	throw "bps_dx_axis_bench.exe was not found. Build it first, for example: cmake --build build\Win-perf --config $Config --target bps_dx_axis_bench"
}
...
try {
	...
	& $benchExe @args
	if ($LASTEXITCODE -ne 0) {
		throw "bps_dx_axis_bench failed with exit code $LASTEXITCODE."
	}
}
finally {
