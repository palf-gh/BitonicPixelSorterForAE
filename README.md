...
validated on Apple Silicon Macs in After Effects 2023 through 2026.
...
GPU backend coverage on Windows: the CUDA backend is verified. The OpenCL and
DirectX backends are built and advertised but have not yet been verified in the
host, so treat them as experimental.
...
Windows の GPU バックエンドの検証状況: CUDA は検証済みです。OpenCL と DirectX は
ビルド・広告されますが、ホスト内では未検証のため experimental（試験的）として
扱ってください。
...
- Windows（Visual Studio 17 2022 x64）および macOS（Xcode）での CMake configure と
  Release / Debug ビルド。
...
Metal 已实现：可编译并链接为通用 `.plugin`，计算内核以运行时编译的 Metal 源码形式
...
Windows 上的 GPU 后端验证情况：CUDA 后端已验证。OpenCL 与 DirectX 后端可构建并
对外声明，但尚未在宿主中验证，请将其视为实验性（experimental）。

### 状态

已验证：

- 在 Windows（Visual Studio 17 2022 x64）与 macOS（Xcode）上的 CMake configure 及
  Release / Debug 构建。
...
Metal은 구현되었습니다. 유니버설 `.plugin`으로 컴파일 및 링크되며, 컴퓨트 커널은
런타임 컴파일되는 Metal 소스로 임베드됩니다. Apple Silicon Mac의 After Effects
2023~2026에서 검증되었습니다.
...
Windows의 GPU 백엔드 검증 상황: CUDA 백엔드는 검증되었습니다. OpenCL과 DirectX
백엔드는 빌드되어 광고되지만 아직 호스트에서 검증되지 않았으므로
실험적(experimental)으로 취급하십시오.

### 상태

검증됨:

- Windows(Visual Studio 17 2022 x64) 및 macOS(Xcode)에서의 CMake configure와
  Release / Debug 빌드.
