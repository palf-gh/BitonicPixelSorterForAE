...
the compute kernel embedded as runtime-compiled Metal source. It has not yet
been validated inside the After Effects host; host validation should be
performed on real Mac hardware.
...
- Metal backend in-host validation (implemented; pending real-Mac AE verification).
...
ただし After Effects ホストでの検証はまだ行われていません。ホスト検証は実機 Mac 上で
実施してください。
...
- Metal バックエンドのホスト内検証（実装済み・実機 Mac の AE 検証待ち）。
...
嵌入。但尚未在 After Effects 宿主中验证；宿主验证应在真实 Mac 硬件上进行。
...
- Metal 后端宿主内验证（已实现，待真实 Mac 的 AE 验证）。
...
런타임 컴파일되는 Metal 소스로 임베드됩니다. 다만 After Effects 호스트에서는 아직
검증되지 않았습니다. 호스트 검증은 실제 Mac 하드웨어에서 수행해야 합니다.
...
- Metal 백엔드 호스트 내 검증(구현 완료, 실제 Mac AE 검증 대기).
